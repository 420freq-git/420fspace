{{-- Satu baris PO. Dipisah supaya bisa dirender ulang dari server saat tahap diubah lewat AJAX
     (lihat PurchaseOrderController::updateStatus) — semua aturan tahap tetap di PHP, tidak
     diduplikasi di JavaScript. --}}
@php
    $tahap = $po->tahap;
    $hari = $po->hari_di_tahap;
    $mandek = ! $tahap->isReady() && $hari !== null && $hari >= 5;
@endphp
<div class="px-5 py-4 flex flex-wrap items-center gap-x-5 gap-y-3" data-po-row="{{ $po->id }}">
    ...
    <div class="flex-1 min-w-[200px]">
        ...
        <div class="mt-1.5 flex items-center gap-2">
            ...
            @endif
            <span data-tahap-status class="text-xs text-brand-600 opacity-0 transition-opacity duration-300"></span>
        </div>
    </div>
