<x-app-layout>
    ...
    <x-slot name="header">
        <div>
            <h1 class="text-lg font-semibold text-sand-900">Settlement / Saldo Vendor</h1>
            <p class="text-xs text-sand-500">Perhitungan kewajiban &amp; pembayaran ke Diferd per batch.</p>
        </div>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-5">
            <div class="rounded-xl border border-sand-200 bg-white shadow-sm p-5">
                <p class="text-sm text-sand-500">Hak Diferd</p>
            ...
            <div class="rounded-xl border border-sand-200 bg-white shadow-sm p-5">
                <p class="text-sm text-sand-500">Sudah dibayar</p>
            ...
            <div class="rounded-xl border border-sand-200 bg-white shadow-sm p-5">
                <p class="text-sm text-sand-500">Saldo ke Diferd</p>
                @if ($totals['saldo'] > 0)
                    <p class="mt-1 text-xl font-semibold text-amber-700 tnum">{{ $fmt($totals['saldo']) }}</p>
                    <p class="mt-1 text-xs text-amber-600">sisa hak yang perlu dibayar</p>
                @else
                    <p class="mt-1 text-xl font-semibold text-brand-700 tnum">{{ $fmt(0) }}</p>
                    <p class="mt-1 text-xs text-brand-600">hak sudah lunas</p>
                @endif
            </div>
            <div class="rounded-xl border border-blue-200 bg-blue-50 shadow-sm p-5" x-data="{ selesaikan: false }">
                <p class="text-sm text-blue-700">Modal di vendor (deposit)</p>
                <p class="mt-1 text-xl font-semibold text-blue-800 tnum">{{ $fmt($totals['modal']) }}</p>
                @if ($totals['modal'] > 0)
                    <p class="mt-1 text-xs text-blue-600">sekali di awal kerja sama · mengendap sampai diselesaikan</p>
                    @if (auth()->user()->isAdmin())
                        <button type="button" @click="selesaikan = ! selesaikan" class="mt-2 text-xs font-medium text-blue-700 hover:underline">Selesaikan…</button>
                        <div x-show="selesaikan" x-cloak class="mt-2 space-y-1.5">
                            <form method="POST" action="{{ route('settlement.deposit.selesaikan') }}"
                                  onsubmit="return confirm('Offset deposit {{ $fmt($totals['modal']) }} ke sisa hak Diferd & tagihan TM? Dilakukan SEKALI di akhir kerja sama — tidak bisa dibatalkan dari sini.');">
                                @csrf @method('PATCH')
                                <input type="hidden" name="cara" value="offset">
                                <button type="submit" class="w-full rounded-md bg-blue-700 px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-blue-800">Offset ke hak Diferd</button>
                            </form>
                            <form method="POST" action="{{ route('settlement.deposit.selesaikan') }}"
                                  onsubmit="return confirm('Tandai deposit {{ $fmt($totals['modal']) }} dikembalikan oleh Diferd? Tidak mengubah hak, tagihan, maupun kas.');">
                                @csrf @method('PATCH')
                                <input type="hidden" name="cara" value="kembali">
                                <button type="submit" class="w-full rounded-md border border-blue-300 bg-white px-2.5 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-100">Dikembalikan Diferd</button>
                            </form>
                        </div>
                    @endif
                @else
                    <p class="mt-1 text-xs text-blue-600">sudah diselesaikan / tidak ada</p>
                @endif
            </div>
            <div class="rounded-xl border border-sand-200 bg-white shadow-sm p-5">
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-sand-200 text-left text-xs uppercase tracking-wide text-sand-500">
                                ...
                                <th class="px-5 py-3 font-semibold text-right">Dibayar</th>
                        ...
                        <tbody class="divide-y divide-sand-100">
                            @foreach ($rows as $row)
                                @php $b = $row['batch']; $s = $row['sum']; @endphp
                                <tr class="hover:bg-sand-50/50">
                                    ...
                                    <td class="px-5 py-3.5 text-right tnum text-brand-700">{{ $fmt($s['fee420f']) }}</td>
        ...
        <div class="space-y-1 text-xs text-sand-400">
            ...
            <p>Modal di vendor (deposit) = dana yang TM salurkan <span class="font-medium">sekali di awal kerja sama</span> langsung ke Diferd — di luar kas 420F, bukan hutang vendor, dan tidak memotong hitungan batch mana pun. Diselesaikan sekali di akhir kerja sama: di-offset ke sisa hak Diferd, atau dikembalikan.</p>
        </div>
    </div>
</x-app-layout>
